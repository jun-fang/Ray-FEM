...
%% need to modify it with rectangular domain
...
% n = round(sqrt(N));          % number of grid points on each dimension
...
m = round((xmax-xmin)/h + 1);  % number of grid points on x dimension
n = round((ymax-ymin)/h + 1);  % number of grid points on y dimension
xyn(:,1) = min(xyn(:,1),m-2);        
xyn(:,2) = min(xyn(:,2),n-2);        
