function [angs,r] = NMLA_2D_correction(x0,y0,c0,omega,Rest,node,elem,u,ux,uy,pde,pct,Nray,data,opt,plt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% poly = [1,0,1,-1.5-0.56*(omega*Rest)^0.75];
% poly = [1,0,1,-2.5-0.93*(omega*Rest)^0.763];
% poly = [1,0,1,-1.5-0.775*(omega*Rest)^0.5];
poly = [1,0,1,-1.5-0.85*(omega*Rest)^0.5];
rt = roots(poly);
pidx = find(rt>0);
r = rt(pidx(1))^3/omega;     %% real radius of observation circle
kr = r*omega/c0;            %% k*r0
L = round(kr + (kr)^(1/3) -2.5);    %% truncation level to obtain needed precision
L = max(1,L);
M = 2*(10*L) + 1;             %% number of samples on the observation circle
...
if (strcmp(data,'num'))      %% use numerically computed data
    ...
    Field = interpolation(node,elem,xynode,u);
    DUx = interpolation(node,elem,xynode,ux);
    DUy = interpolation(node,elem,xynode,uy);
