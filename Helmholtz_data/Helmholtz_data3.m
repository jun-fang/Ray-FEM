...
pde = struct(...
    ...
    function u =  ex_u(p)
        ...
        k = omega;
        x = p(:,1);    y = p(:,2);
        u = besselh(0,1,k*sqrt((x+2).^2 + (y+2).^2))...
            + 2*besselh(0,1,k*sqrt((x-2).^2 + (y-2).^2))...
            + 0.5*besselh(0,1,k*sqrt((x+2).^2 + (y-2).^2))...
            - besselh(0,1,k*sqrt((x-2).^2 + (y+2).^2));
        u = sqrt(k)*u;
    end
    ...
    function phase =  phase(p)
        ...
        phase(:,2) = sqrt((x-2).^2 + (y+2).^2);
        phase(:,3) = sqrt((x-2).^2 + (y-2).^2);
        phase(:,4) = sqrt((x+2).^2 + (y-2).^2);
    end
    ...
    function ray_ang = ray_ang(p)
        ...
        ray_ang(:,1) = atan2(y+2,x+2);
        ray_ang(:,2) = atan2(y+2,x-2);
        ray_ang(:,3) = atan2(y-2,x-2);
        ray_ang(:,4) = atan2(y-2,x+2);
    ...
    function Du =  Du(p)
        ...
        Dux = - besselh(1,1,k*r1).*k.*(x+2)./r1...
            + besselh(1,1,k*r2).*k.*(x-2)./r2...
            - 2*besselh(1,1,k*r3).*k.*(x-2)./r3...
            - 0.5*besselh(1,1,k*r4).*k.*(x+2)./r4;
        Duy = - besselh(1,1,k*r1).*k.*(y+2)./r1...
            + besselh(1,1,k*r2).*k.*(y+2)./r2...
            - 2*besselh(1,1,k*r3).*k.*(y-2)./r3...
            - 0.5*besselh(1,1,k*r4).*k.*(y-2)./r4;
