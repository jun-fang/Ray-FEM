function [u,A,v,b] = Ray_FEM_PML_22_PointSource(node,elem,omega,wpml,sigmaMax,xs,ys,speed,ray,fquadorder,plt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% FEM set up and ray information
...
xn = round((xs - xmin)/h);
yn = round((ys - ymin)/h);
xyn = n*xn + yn + 1;           % location of the point source 
bn = ray_dof(xyn);             % corresponding column number of the point source 
            ...
            for nii = 1: ni
                ...
                for j = 1:3
                    ...
                    for njj = 1: nj
                        ...
                        Mij = weight(p)*sxy*phi(p,i)*phi(p,j).*exp_phase.*area(nt);
...
A = sparse(rows,cols,vals1,Ndof,Ndof);
M = sparse(rows,cols,vals2,Ndof,Ndof);
