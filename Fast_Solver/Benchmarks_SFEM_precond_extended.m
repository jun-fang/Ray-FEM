...
for mult = 1:nSamples
    ...
    h = 1./(20*round(omega*NPW/(2*pi*20)));
    
    
    % physical width of the PML
    wpml = 2*pi/omega + 2*log(mult)*h;
    npml = round(ceil(wpml/h))
    % maximum absorption of the PML
    sigmaMax = 25/wpml;
    
    % Number of subdomains
    nSub = round(omega / (10*pi));
...
save('benchmark_data_SFEM_precond_extended.mat', 'timeOff', 'timeOn', 'NumdeegresFredoom','NumberWavelengths')
