...
fprintf('Solving the traces for the low frequency')
...
u_smooth = uGmres(1:end/2) + uGmres(end/2+1:end);
...
UsolArray = DDM.solveLocal(fInt, u_smooth);
...
fprintf('Solving the high_frequency System\n')
...
if  plt == 1
    ...
    u_smooth = u1.*exp(1i*kk(:).*temp);
    u_smooth = reshape(u_smooth,N,Nray);
    u_smooth = sum(u_smooth,2);

    % showing the real part of the solution
    figure(2); 
    MHigh.showresult(real(u_smooth));

end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Reconstructing the total wavefield
