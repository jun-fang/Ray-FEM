...
epsilon = 1/(4*pi);                 % cut-off parameter   speed = @(x) ones(size(x,1),1);    % medium speed
...
f = assemble_RHS_RayFEM_with_ST(node,elem,xs,ys,omega,epsilon,...
...
save('benchmark_data_RAY_FEM_precond_wave5a.mat', 'timeOff', 'timeOn', 'NumdeegresFredoom','NumberWavelengths')
