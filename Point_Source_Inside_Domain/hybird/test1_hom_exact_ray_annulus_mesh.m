...
n_test = 4;
omegas = [40 80 120 160]*pi;

max_err = zeros(n_test,1);
rel_max_err = zeros(n_test,1);
l2_err = zeros(n_test,1);
rel_l2_err = zeros(n_test,1);
...
for ni = 1:n_test
    ...
    max_err(ni) = norm(du,inf);
    rel_max_err(ni) = norm(du,inf)/norm(eu,inf);
    l2_err(ni) = norm(du)*h;
    rel_l2_err(ni) = norm(du)/norm(eu);
...
x = omegas';
y1 = max_err; y2 = rel_max_err;
y3 = l2_err; y4 = rel_l2_err;
%%
figure(1);
subplot(2,2,1);
FJ_showrate(x,y1);
