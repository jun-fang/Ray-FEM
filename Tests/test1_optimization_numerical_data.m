...
%         ch = 1/(NPW*4*round((sqrt(high_omega))/(2*pi*4)));
    ...
    for i = 1:cN
        x0 = cnode(i,1);  y0 = cnode(i,2);
        c0 = pde.speed(cnode(i,:));
        [oangs1,oBs1] = NMLA_2D_2nd(x0,y0,c0,omega,Rest,mnode,melem,uh1,ux,uy,pde,1/5,Nray,'num',0,plt);
        %     [oangs2,oBs2] = NMLA_2D_2nd(x0,y0,c0,omega,Rest,rnode,relem,ru,ux,uy,pde,pct,Nray,data,opt,plt);
%         [angs1,Bs1] = gradient_descent(mnode,uh1,omega,x0,y0,c0,oangs1,oBs1);
        cnumray(i,:) = oangs1;
