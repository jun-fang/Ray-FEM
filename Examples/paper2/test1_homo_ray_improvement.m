...
function [] = test1_homo_ray_improvement(NPW, test_num)
...
omega1 = high_omega;
omega2 = 0.65*high_omega;

% omega1 = high_omega + 50;
% omega2 = high_omega - 50;
...
high_max_rayerr1 = 0*high_omega;    % L_inf ray error of high-freq waves
high_l2_rayerr1 = 0*high_omega;     % L_2 ray error of high-freq waves
high_max_rayerr2 = 0*high_omega;    % L_inf ray error of high-freq waves
high_l2_rayerr2 = 0*high_omega;     % L_2 ray error of high-freq waves
...
% width of PML
high_wpml = 0.055*ones(size(high_omega));
low_wpml = 0.13*ones(size(high_omega));
...
for ti = 1: test_num
    ...
    numray = numray.*(mr > 2*epsilon) + exray.*(mr <= 2*epsilon);
    rayerr_low = numray - exray;
    low_max_rayerr(ti) = norm(rayerr_low,inf);
    low_l2_rayerr(ti) = norm(rayerr_low)*h/(norm(exray.*(mr > 2*epsilon))*h);
    
    
    %% Step 3: Solve the original Helmholtz equation by Ray-based FEM with ray directions d_c
    fprintf(['-'*ones(1,80) '\n']);
    fprintf('Step3: Ray-FEM, high frequency omega1 \n');
    ...
    fprintf(['-'*ones(1,80) '\n']);
    fprintf('Step4: NMLA, high frequency omega1\n');
    ...
    numray1 = numray1.*(sr > 2*epsilon) + exray.*(sr <= 2*epsilon);
    rayerr1 = numray1 - exray;
    high_max_rayerr1(ti) = norm(rayerr1,inf);
    high_l2_rayerr1(ti) = norm(rayerr1)*h/(norm(exray.*(sr > 2*epsilon))*h);    
    
    
    %% Step 5: Solve the original Helmholtz equation by Ray-based FEM with ray directions d_c
    fprintf(['-'*ones(1,80) '\n']);
    fprintf('Step5: Ray-FEM, high frequency omega2 \n');
    ...
    %% Step 6: NMLA to find original ray directions d_o with wavenumber k
    fprintf(['-'*ones(1,80) '\n']);
    fprintf('Step6: NMLA, high frequency omega1\n');
    ...
    high_max_rayerr2(ti) = norm(rayerr2,inf);
    high_l2_rayerr2(ti) = norm(rayerr2)*h/(norm(exray.*(sr > 2*epsilon))*h);    
    
    
    
    %% ray improvement
    ray = (sqrt(omega1(ti)).*numray1 - sqrt(omega2(ti)).*numray2)./(sqrt(omega1(ti)) - sqrt(omega2(ti)));
    ray = ray./abs(ray);
    ray(sr <= 2*epsilon) = exray(sr <= 2*epsilon);
    rayerr_high = ray - exray;
    high_max_rayerr(ti) = norm(rayerr_high,inf);
    high_l2_rayerr(ti) = norm(rayerr_high)*h/(norm(exray.*(sr > 2*epsilon))*h);    
    
    
    
    %% Step 7: Solve the original Helmholtz equation by Ray-based FEM with ray directions d_o
    fprintf(['-'*ones(1,80) '\n']);
    fprintf('Step7: Ray-FEM, high frequency \n');
    tic;
...
%% save output 
% nameFile = strcat('results_1_HomNumRay_NPW_', num2str(NPW), '.mat');
...
fprintf( '\n\nHigh max ray error1:     ');
fprintf( '&  %1.2d  ',high_max_rayerr1);
fprintf( '\n\nHigh rel l2 ray error1:  ');
fprintf( '&  %1.2d  ',high_l2_rayerr1);

fprintf( '\n\nHigh max ray error2:     ');
fprintf( '&  %1.2d  ',high_max_rayerr2);
fprintf( '\n\nHigh rel l2 ray error2:  ');
fprintf( '&  %1.2d  ',high_l2_rayerr2);
