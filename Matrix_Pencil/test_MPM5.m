...
%         
        ...
        if est_ang >= pi/2 && est_ang < 3*pi/4
            err1(i,4) = abs(2*pi - (exray(i,4) - est_ang) - acos(1i*imag(log(z(1)))/(1i*omega*h)) );
            err1(i,3) = abs(exray(i,3) - est_ang - acos(1i*imag(log(z(2)))/(1i*omega*h)) );
            err1(i,1) = abs(est_ang - exray(i,1) - acos(1i*imag(log(z(3)))/(1i*omega*h)) );
            err1(i,2) = abs(exray(i,2) - est_ang - acos(1i*imag(log(z(4)))/(1i*omega*h)) );
        end
...
%     rec_err3(j) = norm(err1(:,3))*h/(norm(exray(:,3))*h);
%     rec_err4(j) = norm(err1(:,4))*h/(norm(exray(:,4))*h);
