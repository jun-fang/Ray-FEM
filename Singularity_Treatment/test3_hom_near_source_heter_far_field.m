%% Point source problem in heterogeneous medium
% homogeneous near the source and heterogeneous in far-field
clear;


%% Load source data
global omega xs ys a;
xs = -0.35; ys = -0.35;                     % point source location

% distance to the source point
xys = @(p) sqrt( ( p(:,1)-xs).^2 + (p(:,2)-ys).^2);
% medium speed
rs = 0.3;  alpha = 2;
%speed = @(p) 1 + alpha*sin(2*pi*p(:,1)).*( xys(p)> rs).* exp( - 1./(abs( xys(p)- rs ))); 
speed = @(p) 1 + alpha*sin(2*pi*p(:,1)+0.5).*cos(2*pi*p(:,2)-0.5).*( xys(p)> rs).* exp( - 1./(abs( xys(p)- rs )));
pde = [];
...
% frequency
high_omega = [160 40 80 120 160 240 320]*pi;
...
for ti = 1: test_num
    ...
    x_eps = cutoff(epsilon,2*epsilon,node,xs,ys);
    v = ub.*x_eps;
    us = u;
    uu = us + v;
    toc;
